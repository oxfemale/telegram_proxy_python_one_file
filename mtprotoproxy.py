...
def init_config():
    ...
    conf_dict.setdefault("MASK", False)
    ...
    conf_dict.setdefault("SOCKS5_PORT", "9050")
    ...
    conf_dict.setdefault("STATS_PRINT_PERIOD", 120)
...
def make_metrics_pkt(metrics):
    ...
    pkt_header_list.append("Content-Type: text/plain; version=1.3.3.7; charset=utf-8")
...
async def stats_printer():
    ...
    while True:
        ...
        infa = ""
        last_write = open('info.txt', 'w')
        print("Stats for %s" % ( time.strftime("%d.%m.%Y %H:%M:%S")))
        ...
        for user, stat in user_stats.items():
            ...
            info = info + "%s: %d connects (%d current), %.2f MB, %d msgs\n" % (
                user, stat["connects"], stat["curr_connects"],
                (stat["octets_from_client"] + stat["octets_to_client"]) / 1000000,
                stat["msgs_from_client"] + stat["msgs_to_client"])
            ik = 1
        ...
        if last_clients_with_same_handshake:
            ...
            dor = "Clients with duplicate handshake (likely replay-attackers):"
            shu = ""
    ...
    for user, secret in sorted(config.USERS.items(), key=lambda x: x[0]):
        for ip in ip_addrs:
            if config.MODES["classic"]:
                ...
                print("Classic {}: {}".format(user, classic_link), flush=True)
                infa = "{}".format(classic_link)
                # print(infa)
                f = open('classic.txt', 'w')
                f.write(infa)
                f.close()
            ...
            if config.MODES["secure"]:
                ...
                print("Secure {}: {}".format(user, dd_link), flush=True)
                infa = "{}".format(dd_link)
                # print(infa)
                f = open('secure.txt', 'w')
                f.write(infa)
            ...
            if config.MODES["tls"]:
                ...
                print("Tls {}: {}".format(user, tls_link), flush=True)
                infa = "{}".format(tls_link)
                # print(infa)
                f = open('tls.txt', 'w')
                f.write(infa)
                f.close()
